...
class FallingRocksGame
{
    ...
    private static int HitConsequences(int lives, List<GameObject> dwarf, List<GameObject> rocks)
    {
        if (lives > 1)
        {
            ...
            dwarf.Add(new GameObject() { row = Console.WindowHeight - 1, col = 14,
                ch = "(", color = ConsoleColor.White });
            dwarf.Add(new GameObject() { row = Console.WindowHeight - 1, col = 15,
                ch = "0", color = ConsoleColor.White });
            dwarf.Add(new GameObject() { row = Console.WindowHeight - 1, col = 16,
                ch = ")", color = ConsoleColor.White });
    ...
    private static List<GameObject> moveRocks(List<GameObject> rocks, Random randomGenerator)
    {
        ...
        {
            rocks.Add(new GameObject() { row = 5, col = randomGenerator.Next(1, Console.WindowWidth - 1),
                ch = "$", color = ConsoleColor.Green });
        }
        else
        {
            rocks.Add(new GameObject() { row = 5, col = randomGenerator.Next(1, Console.WindowWidth - 1),
                ch = "♥", color = ConsoleColor.Red });
        }
        return rocks;
    }
        ...
        if (Console.KeyAvailable)
        {
            ...
            if (userInput.Key == ConsoleKey.LeftArrow && dwarf[0].col > 1)
            {
                ...
                for (int index = 0; index < dwarf.Count; index++)
                {
                    ...
                    newDwarf.Add(new GameObject() { row = dwarf[index].row, col = dwarf[index].col - 1,
                        ch = dwarf[index].ch, color = dwarf[index].color });
                }
                dwarf.Clear();
                dwarf = newDwarf;
            }
